...
QPndman::Repository::Repository(QString const& url, QString const& name, QString const& updates, QDateTime const& timestamp, QString const& version, QList<Package> const& packages, QObject* parent) : QObject(parent), 
  _url(url), _name(name), _updates(updates), _timestamp(timestamp), _version(version), _packages(packages)
{
}

QPndman::Repository::Repository(Repository const& other) : QObject(0), 
  _url(other._url), _name(other._name), _updates(other._updates), _timestamp(other._timestamp), _version(other._version), _packages(other._packages)
{
}
